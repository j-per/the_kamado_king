...
const RecipeCardSection = (props) => {
  ...
  const shrinkArray = data.allContentfulRecipe.edges.filter((currentVal, index) => index < 4);
  ...
  return (
    <section className={styles.mainRecipeSectionWrapper}>
      ...
      <div className={styles.recipeSectionWrapper}>
        ...
        {data.allContentfulRecipe.edges.map(edge => <RecipeCard edge={edge} key={edge.node.id} />)}
      </div>
      <Link to="/recipes" className={styles.allRecipesLink}>View All</Link>
    </section>
  )
